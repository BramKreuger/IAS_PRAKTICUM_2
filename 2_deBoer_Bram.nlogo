...
patches-own [ meanscore newstrategy strategy ]
...
to go
  ...
  [
    ...
    set newstrategy [strategy] of winner
  ]
  ask patches [set strategy newstrategy]
    ask patches [set pcolor item strategy colorlist]
  tick

  ;;;;; gebruik voor testen symmetrie:   ask patches [set strategy item 0 index set pcolor item 0 colorlist] ask n-of 1 patches [set strategy item 1 index set pcolor item 1 colorlist]
end
...
to-report play [some-strategy my-history your-history] ;
  report ifelse-value (random-float 1.0 < (noise / 100)) [random-action][runresult(word some-strategy " my-history your-history")]
end
